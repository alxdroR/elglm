function setupfitting_GLM_adrMod(gg, Stim,fix);
%  setupfitting_GLMbi(gg, Stim,fix);
%
...
global SPNDS MSP MSTM  MMntrp OPRS MAXSIZE
...
if isempty(MSTM)
    buildMSTM = 1;
elseif size(MSTM,1) ~= slen
    buildMSTM = 1;
elseif size(MSTM,2) ~= ncols
    buildMSTM=1;
end
if buildMSTM
...
ndt = round(1/OPRS.dt);
nhtot = (1-fix.ih)*OPRS.nh+OPRS.nh2*nCoupled;
totNums = (nhtot+1)*rlenLi + size(MSTM,2)*slenLi*(1-fix.kt);
...
elseif size(MMntrp,2)~=max(diff(jchunk'))
    % ----- Make time interpolation matrix for stimulus -----------
    MMntrp = makeInterpMatrix2(max(diff(jchunk')),ndt);
