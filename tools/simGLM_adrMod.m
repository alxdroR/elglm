function [tsp,Vmem,Ispk Z] = simGLM_adrMod(glmprs,Stim);
...
if size(glmprs.k,3) > 1  % Run "coupled" GLM model if multiple cells present
    ...
    if nargout <= 2
        ...
        [tsp,Vmem,Ispk Z] = simGLMsingle(glmprs,Stim);
    end
end
    
% ======================================================
function [tsp,Vmem,Ispk Z] = simGLMsingle(glmprs,Stim);
...
if ~isempty(ih) % Interpolate ih current
    %ihthi = [dt:dt:max(glmprs.iht)]';  % time points for sampling
     ihthi = [0:dt:max(glmprs.iht)]';  % time points for sampling
...
while jbin <= rlen
    ...
    if (tspnext >= rrcum(end)) % No spike in this window
        ...
        nxtid = min(find(rrcum>=tspnext));
        Z(nsp,1) = rrcum(nxtid); % ISI in rescaled time
        Z(nsp,2) = tspnext;
